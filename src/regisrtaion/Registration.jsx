...
function Registration() {
	...
	///////////////// ids//////////////////////////
	const ALLOWED_ADMIN_IDS = new Set(["100", "20", "35", "44", "56"]);
	...
	const requiredFilled =
		name && email && pass && rePass && (!isAdmin || adminId);
	const isAdminIdValid = !isAdmin || isAdminIdAllowed(adminId);
	const disableSubmit =
		hasErrors || !requiredFilled || !isAdminIdValid || isLoading;

	return (
		<div className="flex justify-center items-center min-h-screen">
			<div className="oklch(98.5% 0 0) shadow-lg p-8 rounded-2xl w-96 text-white">
				<h3 className="text-center mb-6 text-black font-bold text-2xl">
				...
				<form onSubmit={handleSubmit}>
					...
					{submitErr && (
						<p className="text-red-500 text-xs italic mt-3">{submitErr}</p>
					)}
					{submitOk && (
						<p className="text-green-600 text-sm mt-3">{submitOk}</p>
					)}
				</form>
			</div>
		</div>
